...
/*
 * AcaJoy core function @copyright (c) AcaJoy All Rights Reserved
 * @TIME:2010-12-18
 */
...
/*
 * 分页函数 @param int $count 条目总数 @param int $perlogs 每页显示条数目 @param int $page 当前页码
 * @param string $url 页码的地址 @return unknown
 */
function pagination ($count, $perlogs, $page, $url, $suffix = '')
{
    ...
    if ($page > 6)
        $re = '<a href="' . $url . '1' . $suffix . '" title="首页">&laquo;</a> ...' .
                 $re;
    if ($page + 5 < $pnums)
        $re .= '... <a href="' . $url . $pnums . $suffix .
                 '" title="尾页">&raquo;</a>';
    if ($pnums <= 1)
        $re = '';
    return $re;
...
/*
 * 写入文件，支持Memcache缓存 @$file 缓存文件 @$dir 缓存目录 @$data 内容
 */
...
/*
 * 封装一个采集函数 @ $url 网址 @ $proxy 代理 @ $timeout 跳出时间
 */
...
function template ($file, $plugin = '', $self = '')
{
    ...
    if ($plugin) {
        ...
        if ($self) {
            ...
            $objfile = 'cache/template/self/' . $self[3] . '.' . $file .
                     '.tpl.php';
        } else {
...
/**
 * 执行挂在钩子上的函数,支持多参数 eg:doAction('post_comment', $author, $email, $url,
 * $comment);
 *
 * @param string $hook            
 *
...
/*
 * AcaJoy专用图片截图函数 $file：数据库里的图片url $app：app名称 $w：缩略图片宽度 $h：缩略图片高度 $c:1裁切,0不裁切
 */
...
/*
 * TS专用删除缓存图片 $file：数据库里的图片url $app：app名称 $w：缩略图片宽度 $h：缩略图片高度
 */
...
/*
 * tsUrl() tsUrl提供至少4种的url展示方式 (1)index.php?app=group&ac=topic&topicid=1
 * //标准默认模式 (2)index.php/group/topic/topicid-1 //path_info模式
 * (3)group-topic-topicid-1.html //rewrite模式1 (4)group/topic/topicid-1
 * //rewrite模式2
 */
function tsUrl ($app, $ac = '', $params = array())
{
    ...
    if ($urlset == 1) {
        ...
        if ($ac == '') {
            ...
            if ($TS_CF['subdomain'] && in_array($app, 
                    $TS_CF['subdomain']['app']) || $TS_CF['appdomain'][$app]) {
                $ac = '?ac=' . $ac;
            } else {
                $ac = '&ac=' . $ac;
            }
    ...
    if ($TS_CF['subdomain'] && in_array($app, $TS_CF['subdomain']['app'])) {
        return 'http://' . $app . '.' . $TS_CF['subdomain']['domain'] . '/' .
                 $url;
...
/*
 * AcaJoy专用上传函数 $file 要上传的文件 如$_FILES['photo'] $projectid 上传针对的项目id 如$userid
 * $dir 上传到目录 如 user $uptypes 上传类型，数组 array('jpg','png','gif')
    ...
    if ($files['size'] > 0) {
        ...
        if (in_array($type, $uptypes)) {
            ...
            move_uploaded_file($files['tmp_name'], 
                    mb_convert_encoding($dest, "gb2312", "UTF-8"));
...
function urlcontent ($content)
{
    ...
    $content = preg_replace($pattern, 
            '<a rel="nofollow" target="_blank" href="\1\2">\1\2</a>', $content);
    return $content;
}
...
{
    return $value = is_array($value) ? array_map('tsadds', $value) : addslashes(
            $value);
}
...
function cleanJs ($text)
{
    ...
    // 过滤多余html
    $text = preg_replace(
            '/<\/?(html|head|meta|link|base|body|title|style|script|form|iframe|frame|frameset)[^><]*>/i', 
            '', $text);
    // 过滤on事件lang js
    while (preg_match(
            '/(<[^><]+)(lang|onfinish|onmouse|onexit|onerror|onclick|onkey|onload|onchange|onfocus|onblur)[^><]+/i', 
            $text, $mat)) {
