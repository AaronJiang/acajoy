...
switch ($ts) {
    ...
    case "do":
        ...
        foreach ($_POST['option'] as $key => $item) {
            ...
            $new['system']->create('system_options', 
                    array(
        ...
        $arrOptions = $new['system']->findAll('system_options', null, null, 
        ...
        // 生成伪静态文件
        if ($_POST['option']['site_urltype'] == 3 ||
                 $_POST['option']['site_urltype'] == 4 ||
                 $_POST['option']['site_urltype'] == 5 ||
                 $_POST['option']['site_urltype'] == 6 ||
                 $_POST['option']['site_urltype'] == 7) {
            ...
            if (! is_writable(JOYROOT . '/.htaccess'))
                qiMsg(
                        "文件(.htaccess)不可写。如果您使用的是Unix/Linux主机，请修改该文件的权限为777。如果您使用的是Windows主机，请联系管理员，将此文件设为everyone可写");
            $htaccess = "RewriteEngine On\n" . "RewriteBase " . $scriptName[0] .
                     "\n" . "RewriteRule ^index\.php$ - [L]\n" .
                     "RewriteCond %{REQUEST_FILENAME} !-f\n" .
                     "RewriteCond %{REQUEST_FILENAME} !-d\n" . "RewriteRule . " .
                     $scriptName[0] . "index.php [L]\n" .
