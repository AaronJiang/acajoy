...
class group extends tsApp
{
    ...
    function getOneGroup ($groupid)
    {
        ...
        if ($strGroup['groupicon'] == '') {
            ...
            $strGroup['icon_48'] = SITE_URL .
                     tsXimg($strGroup['groupicon'], 'group', 48, 48, 
                            $strGroup['path'], 1);
            $strGroup['icon_16'] = SITE_URL .
                     tsXimg($strGroup['groupicon'], 'group', 16, 16, 
                            $strGroup['path'], 1);
        }
    ...
    function getGroupContent ($page = 1, $prePageNum, $groupid)
    {
        ...
        $arrGroupContent = $this->db->fetch_all_assoc(
                "select * from " . dbprefix .
                         "group_topics where groupid='$groupid' order by addtime desc $limit");
    ...
    {
        $arrRecommendGroups = $this->db->fetch_all_assoc(
                "select groupid from " . dbprefix .
                         "group where isrecommend='1' limit $num");
    ...
    {
        $arrNewGroups = $this->db->fetch_all_assoc(
                "select groupid from " . dbprefix .
                         "group where `isaudit`='0' order by addtime desc limit $num");
    ...
    {
        $strGroupContent = $this->db->once_fetch_assoc(
                "select * from " . dbprefix .
                         "group_topics where topicid=$topicid");
    ...
    {
        $strComment = $this->db->once_fetch_assoc(
                "select * from " . dbprefix .
                         "group_topics_comments where commentid='$referid'");
    ...
    {
        $isTopic = $this->findCount('group_topics', 
                array(
                        'topicid' => $topicid
                ));
    ...
    {
        $strTopic = $this->find('group_topics', 
                array(
                        'topicid' => $topicid
                ));
    ...
    function newTopic ($groupid = null, $limit)
    {
        ...
        $arrTopics = aac('group')->findAll('group_topics', $conditions, 
    ...
    public function topicAfter ($topicid)
    {
        ...
        $arrAfters = $this->findAll('group_topics_add', 
                array(
                        'topicid' => $topicid
                ));
        ...
        foreach ($arrAfters as $key => $item) {
            ...
            $arrAfter[$key]['title'] = htmlspecialchars(
    ...
    public function delTopic ($topicid)
    {
        ...
        $this->delete('group_topics', array(
                'topicid' => $topicid
        ));
        $this->delete('group_topics_comments', array(
                'topicid' => $topicid
        ));
        $this->delete('tag_topic_index', array(
                'topicid' => $topicid
        ));
        $this->delete('group_topics_collects', array(
                'topicid' => $topicid
        ));
    ...
    {
        $arrAfter = $this->findAll('group_topics_add', 
                array(
                        'topicid' => $topicid
                ));
    ...
    {
        $arrComment = $this->findAll('group_topics_comments', 
                array(
                        'topicid' => $topicid
                ));
    ...
    {
        $strComment = $this->find('group_topics_comments', 
                array(
                        'commentid' => $commentid
                ));
        ...
        $this->delete('group_topics_comments', 
                array(
                        'commentid' => $commentid
                ));
    ...
    public function hotTopics ($day, $num = 10)
    {
        ...
        $arrTopic = $this->findAll('group_topics', 
                "`addtime`>'$startTime' and `addtime` < '$endTime' and `isshow`='0' and `isaudit`='0'", 
                'count_comment desc', null, $num);
    ...
    {
        $strLike['num'] = $this->findCount('group_topics_collects', 
                array(
                        'topicid' => $topicId
                ));
        
        $strLike['topic'] = $this->find('group_topics', 
                array(
                        'topicid' => $topicId
                ));
        
        $likeUsers = $this->findAll('group_topics_collects', 
                array(
                        'topicid' => $topicId
    ...
    /*
     * 小组针对其他APP发帖内容结合的接口 $groupid 小组ID $userid 用户ID $appkey app目录名称，例如：group
     * $appname APP名称 例如：小组 $appaction APP控制器 例如：topic $appid APP控制器针对的内容id
     * 例如：123 $title 内容标题
     */
    public function appPost ($groupid, $userid, $appkey, $appname, $appaction, 
            $appid, $title)
    {
        if ($groupid && $userid && $appkey && $appname && $appid && $title) {
            $this->create('group_topics', 
                    array(
                            'groupid' => $groupid,
                            'userid' => $userid,
                            'appkey' => $appkey,
    ...
    // 删除小组中针对其他APP发帖内容结合的接口
    public function delappPost ($appkey, $appid)
    {
        if ($appkey && $appid) {
            $this->delete('group_topics', 
                    array(
                            'appkey' => $appkey,
                            'appid' => $appid
                    ));
    ...
    {
        $count_topic_audit = $this->findCount('group_topics', 
                array(
                        'groupid' => $groupid,
                        'isaudit' => '1'
                ));
    ...
    {
        $count_topic = $this->findCount('group_topics', 
                array(
                        'groupid' => $groupid
                ));
    ...
    public function getHotTopic ($day)
    {
        ...
        $arrTopic = $this->findAll('group_topics', $arr, 'addtime desc', 
                'topicid,title', 10);
