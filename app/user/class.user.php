...
class user extends tsApp
{
    ...
    {
        $arrNewUserId = $this->findAll('user_info', null, 'addtime desc', 
                'userid', $num);
    ...
    {
        $arrNewUserId = $this->findAll('user_info', null, 'uptime desc', 
                'userid', $num);
        foreach ($arrNewUserId as $item) {
    ...
    {
        $arrUserId = $this->findAll('user_info', null, 'count_followed desc', 
                'userid', $num);
    ...
    {
        $arrUserId = $this->findAll('user_info', null, 'count_score desc', 
                'userid', $num);
        foreach ($arrUserId as $item) {
    ...
    function getOneUser ($userid)
    {
        ...
        if ($strUser['face'] && $strUser['path']) {
            $strUser['face'] = SITE_URL .
                     tsXimg($strUser['face'], 'user', 48, 48, $strUser['path'], 
                            1);
    ...
    public function isLogin ()
    {
        ...
        if ($userid > 0) {
            ...
            header("Location: " . tsUrl('user', 'login', array(
                    'ts' => 'out'
            )));
            exit();
        }
    ...
    public function getRole ($score)
    {
        ...
        foreach ($arrRole as $key => $item) {
            if ($score > $item['score_start'] && $score <= $item['score_end'] ||
                     $score > $item['score_start'] && $item['score_end'] == 0 ||
                     $score >= 0 && $score <= $item['score_end']) {
    ...
    function toEmpty ($userid)
    {
        ...
        // 是否存在Email
        $isEmail = $this->findCount('anti_email', 
                array(
                        'email' => $strUser['email']
                ));
        if ($isEmail == 0) {
            $this->create('anti_email', 
                    array(
                            'email' => $strUser['email'],
        ...
        $this->delete('article_comment', array(
                'userid' => $userid
        ));
        $this->delete('article_recommend', array(
                'userid' => $userid
        ));
        
        // attach
        $this->delete('attach', array(
                'userid' => $userid
        ));
        $this->delete('attach_album', array(
                'userid' => $userid
        ));
        ...
        $this->delete('user_info', array(
                'userid' => $userid
        ));
        $this->delete('user_follow', array(
                'userid' => $userid
        ));
        $this->delete('user_follow', array(
                'userid_follow' => $userid
        ));
        $this->delete('user_gb', array(
                'userid' => $userid
        ));
        $this->delete('user_open', array(
                'userid' => $userid
        ));
        $this->delete('user_scores', array(
                'userid' => $userid
        ));
        $this->delete('user_score_log', array(
                'userid' => $userid
        ));
        ...
        $this->delete('photo_comment', array(
                'userid' => $userid
        ));
        
        // tag
        $this->delete('tag_user_index', array(
                'userid' => $userid
        ));
    }
