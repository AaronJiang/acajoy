...
$strArticle = $new['article']->find('article', 
        array(
                'articleid' => $articleid
        ));
...
// 是否审核
if ($strArticle['isaudit'] == 1 && $TS_USER['user']['userid'] == 0) {
    tsNotice('文章审核中...');
}

$strArticle['tags'] = aac('tag')->getObjTagByObjid('article', 'articleid', 
        $articleid);
...
$arrComments = $new['article']->findAll('article_comment', 
        array(
                'articleid' => $articleid
...
$commentNum = $new['article']->findCount('article_comment', 
        array(
                'articleid' => $articleid
        ));
...
// 标签
$strArticle['tags'] = aac('tag')->getObjTagByObjid('article', 'articleid', 
        $strArticle['articleid']);

// 推荐阅读
$arrRecommend = $new['article']->getRecommendArticle($strArticle['cateid']);
